defmodule PhoenixComponents.TimelineTest do
  use PhoenixComponents.DataCase
  ...
  describe "posts" do
    ...
    import PhoenixComponents.TimelineFixtures

    @invalid_attrs %{body: nil, username: nil, likes_counts: nil, reposts_count: nil}
    ...
    test "create_post/1 with valid data creates a post" do
      valid_attrs = %{body: "some body", username: "some username", likes_counts: 42, reposts_count: 42}

      assert {:ok, %Post{} = post} = Timeline.create_post(valid_attrs)
      assert post.body == "some body"
      assert post.username == "some username"
      assert post.likes_counts == 42
    ...
    test "update_post/2 with valid data updates the post" do
      post = post_fixture()
      update_attrs = %{body: "some updated body", username: "some updated username", likes_counts: 43, reposts_count: 43}

      assert {:ok, %Post{} = post} = Timeline.update_post(post, update_attrs)
      assert post.body == "some updated body"
      assert post.username == "some updated username"
      assert post.likes_counts == 43
